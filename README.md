# build-dotnet
A reusable workflow for building .NET projects.
...
jobs:
  build:
    uses: Arthri/build-dotnet/.github/workflows/i.yml@v1
```
...
Additional arguments can be passed to the `dotnet build` command using the `build-arguments` input parameter.
```yml
jobs:
  build:
    uses: Arthri/build-dotnet/.github/workflows/i.yml@v1
